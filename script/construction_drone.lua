...
local unique_index = function(entity)
    ...
    return entity.surface.index .. entity.name .. entity.position.x ..
               entity.position.y
end

local is_commandable =
    function(string) return drone_prototypes[string] ~= nil end
...
local get_drone_radius = function() return 0.3 end
...
local get_radius = function(entity, range, goto_entity)
    ...
    elseif is_commandable(entity.name) then
        if range == ranges.interact then
            radius = get_radius_map()[entity.name] +
                         drone_prototypes[entity.name].interact_range
        elseif range == ranges.return_to_character then
            radius = get_radius_map()[entity.name] +
                         drone_prototypes[entity.name].return_to_character_range
        else
            radius = get_radius_map()[entity.name]
        end
    ...
    if radius < oofah then return oofah end
    return radius
end
...
local in_construction_range = function(drone, target)
    ...
    return distance <=
               ((get_radius(drone, ranges.interact) + (get_radius(target))))
end

local floor = math.floor
local random = math.random
local stack_from_product = function(product)
    local count = floor(product.amount or
                            (random() *
...
local get_proxy_chest = function(drone)
    ...
    if proxy_chest and proxy_chest.valid then return proxy_chest end
...
local inventories = function(entity)
    ...
    local inventories = {}
    for k = 1, 10 do inventories[k] = get(k) end
    return inventories
end
...
local transport_lines = function(entity)
    ...
    local inventories = {}
    for k = 1, max_line_index do inventories[k] = get(k) end
    return inventories
end

local is_cheat_mode = function(entity)
    return entity.type == "character" and entity.player and
               entity.player.cheat_mode
end
...
local can_player_spawn_drones = function(player)
    ...
    local count = player.get_item_count(names.units.construction_drone) -
...
local transfer_stack = function(destination, source_entity, stack)
    ...
    stack.count =
        math.min(stack.count, source_entity.get_item_count(stack.name))
    ...
    for k, inventory in pairs(inventories(source_entity)) do
        while true do
            ...
            if source_stack and source_stack.valid and
                source_stack.valid_for_read and can_insert(source_stack) then
...
local transfer_inventory = function(source, destination)
    ...
    for k = 1, #source do
        ...
        if stack and stack.valid and stack.valid_for_read and can_insert(stack) then
            ...
            if remove_stack.count > 0 then remove(remove_stack) end
        end
    end
end
    ...
    if products then
        for k, product in pairs(products) do
            ...
            if stack then
                ...
                if leftover > 0 then
                    to_spill[stack.name] =
                        (to_spill[stack.name] or 0) + leftover
                end
            end
        end
    end
...
remote.add_interface("construction_drone", {
    set_debug = function(bool) data.debug = bool end,
    dump = function()
...
local get_drone_stack_capacity = function(force)
    ...
    drone_stack_capacity = game.entity_prototypes[proxy_name]
...
local validate = function(entities)
    for k, entity in pairs(entities) do
        if not entity.valid then entities[k] = nil end
    end
    return entities
end
...
local make_player_drone = function(player)
    local position = player.surface.find_non_colliding_position(names.units
                                                                    .construction_drone,
                                                                player.position,
                                                                5, 0.5, false)
...
local find_a_player = function(drone_data)
    ...
    if drone_data.player and drone_data.player.valid and
        drone_data.player.surface == entity.surface then return true end
...
local check_ghost = function(entity, player)
    ...
    if entity.name == "tile-ghost" then
        extra = surface.find_entities_filtered {
            type = tile_ghost_type,
            position = position,
            radius = 3
        }
    else
...
local check_proxy = function(entity, player)
    ...
    if data.already_targeted[unique_index(entity)] then return end
...
local check_cliff_deconstruction = function(entity, player)
    ...
    if not cliff_destroying_item then return end

    if player.get_item_count(cliff_destroying_item) == 0 and
        (not player.cheat_mode) then return end
...
local check_deconstruction = function(entity, player)
    ...
    if not (entity.force == force or entity.force.name == "neutral" or
        entity.force.get_friend(force)) then return end
    ...
    if needed <= 1 then
        ...
        for k, nearby in pairs(surface.find_entities_filtered {
            ...
            if should_check then
                ...
                data.already_targeted[nearby_index] = true
                data.sent_deconstruction[nearby_index] =
                    (data.sent_deconstruction[nearby_index] or 0) + 1
                extra_targets[nearby_index] = nearby
                count = count - 1
            end
        end
    ...
    for k = 1, math.min(needed, 10,
                        player.get_item_count(names.units.construction_drone)) do
    ...
    if sent >= needed then data.already_targeted[index] = true end
end
...
local get_repair_items = function()
    ...
    for name, item in pairs(game.item_prototypes) do
        if item.type == "repair-tool" then repair_items[name] = item end
    end
    return repair_items
end
...
local check_repair = function(entity, player)
    ...
    if not repair_item then return end
...
local ignored_types = {
    "resource", "corpse", "beam", "flying-text", "explosion",
    "smoke-with-trigger", "stream", "fire-flame", "particle-source",
    "projectile", "sticker", "speech-bubble"
}
...
local scan_for_nearby_jobs = function(player, area)
    ...
    for k, entity in pairs(entities) do check_entity(entity) end
...
local check_player_jobs = function(player)
    ...
    local count = math.min(5,
                           player.get_item_count(names.units.construction_drone) -
                               (data.request_count[player.index] or 0))
...
local schedule_new_searches = function()
    ...
    for k, player in pairs(game.connected_players) do
        local index = player.index
        if can_player_spawn_drones(player) and
            not next(data.job_queue[index] or {}) then
...
local on_tick = function(event)
    ...
    if event.tick % search_refresh == 0 then schedule_new_searches() end

end

local get_build_time = function(drone_data) return random(15, 25) end
...
local clear_extra_targets = function(drone_data)
    ...
    if order == drone_orders.deconstruct or order ==
        drone_orders.cliff_deconstruct then
        for index, entity in pairs(targets) do
            local index = unique_index(entity)
            data.sent_deconstruction[index] =
                (data.sent_deconstruction[index] or 1) - 1
        end
    end
...
local cancel_drone_order = function(drone_data, on_removed)
    ...
    if not on_removed then set_drone_idle(drone) end
...
local move_to_order_target = function(drone_data, target)
    ...
    if in_construction_range(drone, target) then return true end

    drone.set_command {
        type = defines.command.go_to_location,
        destination_entity = target,
        radius = ((target == drone_data.character and 0) or
            get_radius(drone, ranges.interact)) + get_radius(target, nil, true),
        distraction = defines.distraction.none,
        pathfind_flags = drone_pathfind_flags
    }
...
local move_to_player = function(drone_data, player)
    ...
    if distance(drone.position, player.position) < 2 then return true end
...
update_drone_sticker = function(drone_data)
    ...
    if renderings then
        for k, v in pairs(renderings) do rendering.destroy(v) end
...
local unit_clear_target = function(unit, target)
    ...
    local non_colliding_position = unit.surface.find_non_colliding_position(
                                       unit.name, position, 0, 0.5)
    unit.set_command {
        type = defines.command.go_to_location,
        destination = position,
        radius = 1
...
local get_extra_target = function(drone_data)
    ...
    local next_target = drone_data.entity.surface.get_closest(drone_data.entity
                                                                  .position,
                                                              drone_data.extra_targets)
    if next_target then
...
local process_construct_command = function(drone_data)
    ...
    local drone_inventory = get_drone_inventory(drone_data)
    if drone_inventory.get_item_count(drone_data.item_used_to_place) <
        drone_data.item_used_to_place_count then
        return cancel_drone_order(drone_data)
    end
...
local process_deconstruct_command = function(drone_data)
    ...
    if not drone_data.beam then
        local build_time = get_build_time(drone_data)
        local orientation, offset = get_beam_orientation(drone.position,
                                                         target.position)
        drone.orientation = orientation
    ...
    if target.type == tile_deconstruction_proxy then
        tiles = {
            {
                name = target.surface.get_hidden_tile(target.position) or
                    "grass-1",
    ...
    if tiles then drone.surface.set_tiles(tiles, true, false, false, true) end
...
local process_repair_command = function(drone_data)
    ...
    local repair_cycles_left = math.ceil(
                                   (target.prototype.max_health - target.health) /
                                       repair_speed)
...
local process_upgrade_command = function(drone_data)
    ...
    local type = entity_type == "underground-belt" and
                     target.belt_to_ground_type or
                     (entity_type == "loader" or entity_type == "loader-1x1") and
                     target.loader_type
    local position = target.position
    ...
    if neighbour and neighbour.valid and
        drone_inventory.get_item_count(drone_data.item_used_to_place) > 0 then
        -- print("Upgrading neighbour")
        local type = neighbour.type == "underground-belt" and
                         neighbour.belt_to_ground_type
...
local process_request_proxy_command = function(drone_data)
    ...
    if not move_to_order_target(drone_data, proxy_target) then return end
    ...
    if moduleInv then inserted = moduleInv.insert(stack) end
    ...
    if requests[stack_name] <= 0 then requests[stack_name] = nil end
...
local process_deconstruct_cliff_command =
    function(drone_data)
        ...
        local index = unique_index(target)
        get_drone_inventory(drone_data).remove {
            name = target.prototype.cliff_explosive_prototype,
            count = 1
        }
        target.surface.create_entity {
            name = "ground-explosion",
            position = util.center(target.bounding_box)
        }
        target.destroy()
        data.already_targeted[index] = nil
...
process_drone_command = function(drone_data, result)
    ...
    if drone_data.player and drone_data.player.valid and
        drone_data.player.character then
...
local on_entity_removed = function(event)
    ...
    if drone_data then cancel_drone_order(drone_data, true) end
...
local on_construction_drone_toggle = function(event)
    ...
    if not enabled then data.job_queue[event.player_index] = nil end
end
